...
def mask_rgb_compose(org, mask, mask_color = [255, 0, 0]):  # More vibrant mask color
    alpha = 0.5  # Increase the alpha for more mask dominance
    rgb = np.copy(org)
    rgb[mask] = (rgb[mask] * (1 - alpha) + np.array(mask_color) * alpha).astype(np.uint8)
    
    return Image.fromarray(rgb)


def vis_mask_pil(pil, mask_coarse, crf, bs, sigma_spatial, sigma_luma, sigma_chroma):
    w, h = pil.size
    ...
    if crf:
        ...
        _, mask_refine = bilateral_solver.bilateral_solver_output(np.array(pil), mask_coarse, sigma_spatial, sigma_luma, sigma_chroma)
    else:
