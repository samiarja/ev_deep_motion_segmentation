...
event_name              = f"output/{data}_{seq}"
...
loader                = DataLoader(data, seq, tstart=start_time*1e6, windowsize=(finish_time-start_time)*1e6)
width, height, events = loader.load_data()
...
image_directory = f"../../../ev_deep_motion_segmentation/{event_name}/input_frames"
...
cmd1 = (
    f"./es_to_frames"
    f" -i ../../../../Dataset/es/{data}_{seq}_events.es"
    f" -o {image_directory}"
    f" -s exponential"
    f" -t 100000"
...
cmd2 = (
    ...
    f"   ffmpeg -i \"$file\" -vf 'format=rgb24' \"${{file%.ppm}}.png\" >/dev/null 2>&1;" #'format=rgb24,vflip'
    f" done &&"
    f" rm *.ppm &&"
    f" ls -1 | grep '\.png$' | sort | head -n 2 | xargs -d '\\n' rm --"
)
os.system(cmd2)
os.chdir(os.path.expanduser("../../../ev_deep_motion_segmentation/"))


event_warping.print_message(f'Extracting optical flow with RAFT', color='blue', style='bold')
